...
- **台灣時間 20:00**（晚上 8 點）起跑，並在同晚做多次重試（減少 GitHub schedule 偶發延遲/漏跑、或來源更新較晚造成的缺漏）。
- GitHub 的 `cron` 一律使用 **UTC**：
  - 台灣 **UTC+8** → 20:00 / 20:20 / 20:40 台灣 = **當日 12:00 / 12:20 / 12:40 UTC**
  - 對應 cron：`0 12 * * *`、`20 12 * * *`、`40 12 * * *`
...
例：台灣 20:00 → UTC 12:00 → `0 12 * * *`（目前專案會在 20:00/20:20/20:40 進行重試）。
